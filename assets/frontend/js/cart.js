...
function updateCartDisplay() {
  ...
  if (cart.length === 0) {
    ...
    cart.forEach((item, index) => {
      ...
      cartList.append(`
                  <div class="d-flex align-items-center border-bottom p-2">
                     <img src="${item.image
        }" alt="Product" class="me-2" style="width: 60px; height: 60px;">
                     <div class="flex-grow-1">
                           <a class="mb-1 product-title">${item.name}</a>
                           <p class="text-danger">${item.quantity
        } x $${parseFloat(item.price).toFixed(2)}</p>
                     </div>
    ...
    $(".shipping_checkout").html(""); // Clear shipping methods once
...
function add_to_cart(element) {
  if (!element) {
    console.error("add_to_cart() was called without an element!");
    return;
  }

  if ($('#related_product').css('display') === 'none') {
  ...
  let baseUrl = element.getAttribute("pbaseurl");
  let cat_id = element.getAttribute("cat_id");
  let configuration = [];

  if (!productId || !productName || !productPrice || !baseUrl) {
    console.error("Missing product attributes in add_to_cart().");
    return;
  }
  ...
  const configSignature = JSON.stringify(configuration);

  // Check if same product ID AND same config already exists
  let existingItem = cart.find(item =>
    item.id === productId &&
    JSON.stringify(item.configuration || []) === configSignature
  );
  ...
  if (element.type === "checkbox") {
    if (element.checked) {
      ...
      cart = cart.filter(item =>
        !(item.id === productId && JSON.stringify(item.configuration || []) === configSignature)
      );
    }
  } else {
    // Always push as new if different config
    if (existingItem) {
  ...
  Swal.fire({
    title: element.checked || element.tagName === "BUTTON" ? "Added to Cart!" : "Removed from Cart!",
    text: `${productName} has been ${element.checked || element.tagName === "BUTTON" ? "added to" : "removed from"} your cart.`,
    icon: "success",
    toast: true,
    position: "top-end",
    ...
    success: function (response) {
      ...
      $('#product-container-auto').append(`
        <div class="seemoreDiv">
          <a href="/autosearch?q=${input_query}">
